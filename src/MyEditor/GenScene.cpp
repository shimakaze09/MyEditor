...
Scene* GenScene_0() {
  ...
  auto [wall_left, geo_wall_left, mat_wall_left] =
      scene->CreateSObj<Cmpt::Geometry, Cmpt::Material>("wall_left",
                                                        cornellbox);
  ...
  auto [wall_down, geo_wall_down, mat_wall_down] =
      scene->CreateSObj<Cmpt::Geometry, Cmpt::Material>("wall_down",
                                                        cornellbox);
  auto [wall_back, geo_wall_back, mat_wall_back] =
      scene->CreateSObj<Cmpt::Geometry, Cmpt::Material>("wall_back",
                                                        cornellbox);
...
Scene* My::GenScene(size_t n) {
  assert(n < 1);

  using Func = Scene*();
  Func* funcs[] = {&detail::GenScene_::GenScene_0};
